...
function initJQuery() {
    ...
    if (typeof(jQuery) == 'undefined') {
        ...
        if (! jQueryScriptOutputted) {
            ...
            document.write("<scr" + "ipt type="text/javascript" src="//ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js"></scr" + "ipt>");
        }
...
<style type="text/css">
.translation-dialog ul{
	text-align: left;
}

.translation-dialog ul li span{
	width: 20px;
...
<script>
        $(document).ready(function(){
            $('.translation-editable').editable().on('hidden', function(e, reason){
            	var locale = $(this).data('locale');
                if(reason === 'save'){
                    $(this).removeClass('status-0').addClass('status-1');
                }
                if(reason === 'save' || reason === 'nochange') {
                    var $next = $(this).closest('li').next().find('.editable');
                    setTimeout(function() {
                        $next.editable('show');
                    }, 300);
                }
	        ...
	        $('.translation-edit').on('click', function(){
	        ...
	        $('.form-publish-and-refresh').on('ajax:success', function (e, data) {
	        	location.reload();
            });
        })
</script>